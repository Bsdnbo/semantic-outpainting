...
def save_network(net, label, epoch, opt):
    ...
    save_filename = '%s_net_%s_%s.pth' % (epoch, label, opt.name)
    print("Saving %s to %s" % (save_filename, opt.checkpoints_dir + '/' + save_filename), flush=True)
    torch.save(net.state_dict(), opt.checkpoints_dir + '/' + save_filename)
    print("Saved!", flush=True)
    # if opt.use_gpu and torch.cuda.is_available():
    #     net.cuda()


def load_network(net, save_filename, opt):
    if opt.isTrain:
